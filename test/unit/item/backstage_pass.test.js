...
describe("`BackstagePass` - child class of `RegularItem`", () => {
  ...
  it("updateItemQuality() increases `quality` more than by 1 based on `sellIn` value. Max quality = 50", () => {
    // Scenario 1
  ...
  it("updateItemQuality() changes `quality` to 0 when `sellIn` value <= 0", () => {
    const backstagePass1 = new BackstagePass("Backstage Pass 1", 1, 50);

    expect(backstagePass1.name).toEqual("Backstage Pass 1");
    expect(backstagePass1.sellIn).toEqual(1);
    expect(backstagePass1.quality).toEqual(50);
    ...
    expect(backstagePass1.sellIn).toEqual(0);
    ...
    expect(backstagePass1.sellIn).toEqual(0);
    expect(backstagePass1.quality).toEqual(0);
  });
});
